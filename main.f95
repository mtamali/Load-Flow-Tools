...
character(50) :: A2Fmt, AFmt, A3Fmt, fnamein, fnameout, name, hlpfilename = "NR.hlp"
...
If (.not. lookForCDF) Then
    ...
    write(60,*) 'le nombre de noeuds = ', trim(Str(N)), ', le nombre de branches = ', trim(Str(NB))
...
Do while (Cond==0 .and. k<=Kmax)
    ...
    do i = 1, N-1
       do j = N, JN
          if (Indx(i+1) == Indx(j-Ng)) then
              Ja(i, j) = DP_DModVii(Indx(i+1), N)
          else
              Ja(i, j) = DP_DModVij(Indx(i+1), Indx(j-Ng))
          end if
       end do
    enddo
    ...
    do i = N, JN
       do j = 1, N-1
          if (Indx(i-Ng) == Indx(j+1)) then
              Ja(i, j) = DQ_Dtetaii(Indx(i-Ng), N)
          else
              Ja(i, j) = DQ_Dtetaij(Indx(i-Ng), Indx(j+1))
          endif
       end do
    enddo
    ...
    do i = N, JN
       do j = N, JN
          if (Indx(i-Ng) == Indx(j-Ng)) then
              Ja(i, j) = DQ_DModVii(Indx(i-Ng), N)
          else
              Ja(i, j) = DQ_DModVij(Indx(i-Ng), Indx(j-Ng))
          endif
       end do
    enddo
    ...
    do I = N, JN
           Fd(I) = - Q(Indx(I-Ng)) + CalcQ(Indx(I-Ng))
    enddo
...
!
    ...
    if (MaxDX <= eps) then
        ...
        write(60, "(A,F6.2,A5)") "            Tableau des puissances nodales  (pu), Sbase = ", Sbase, " MVA."
        write(60, *) "------------------------------------------------------------------------------------"
...
contains
    ...
    Subroutine InvMat(Aa, Cc, Nn)
        ...
        Real :: L(Nn,Nn), U(Nn,Nn), b(Nn), d(Nn), Xx(n)
        Real :: coeff
        integer ii, ij, ik
...
!        fm = "(" // Str(Ng+1) // "(I3,2X))"
!        write(60, fm) (Indx(i_), i_=1,N)

    end Subroutine Mef

    Subroutine SaveImport()
        implicit none
        integer :: i_, j_, k_

        open (unit=70, file="/home/mtamali/NR/NR/tmp.dat", action="write")
        write(70, "(A,1X,I4,1X,I4)") trim(NName), N, NB
        do i_=1,NB
!           j_ = Indx(Nfrom(i_))
!           k_ = Indx(Nto(i_))
           j_ = Nfrom(i_)
           k_ = Nto(i_)
           write(70, "(I4,1X,I4,1X,2F10.4,1X,2F10.4, I3,1X,I3)") Indx(j_), Indx(k_), Z(j_, k_), Ys(j_, k_), j_, k_
        enddo
        do j_ = 1, N
           i_ = Indx(j_)
           if (Typen(i_) == "B") Then
